...
var addpost=function(req,res){
    console.log('post 모듈안에 addpost호출됨.');
    ...
    if(database.db){
        ...
        database.UserModel.findByEmail(paramWriter,function(err,results){
            if(err){
                console.error('게시판 글 추가 중 오류발생:'+err.stack);
                
                
                res.writeHead('200',{'Content-Type':'text/html;charset=utf8'});
                res.write('<h2>게시판 글 추가중 오류발생.</h2>');
            ...
            if(results==undefined || results.length<1){
                ...
                res.writeHead('200',{'Content-Type':'text/html;charset=utf8'});
                res.write('<h2>사용자['+paramWriter+']를 찾을수없습니다.</h2>');
                res.end();
            ...
            var post=new database.PostModel({
                ...
                post.savePost(function(err,result){
                    if(err){ 
                        ...
                        console.log('글 데이터 추가!');
                        console.log('글작성','포스팅 글을 생성했습니다.:'+post._id);
                        res.end();
...
var showpost=function(req,res){
	...
	if (database.db) {
		...
		database.PostModel.load(paramId, function(err, results) {
            console.log('등록된 글의 list>>'+results);
			if (err) {
...
var listpost=function(req,res){
  ...
  if(database.db){
      ...
      database.PostModel.list(options,function(err,results){
          if(err){
              console.error('게시판 글 목록 조회 중 오류발생:'+err.stack);
          ...
          if(results){
              ...
              database.PostModel.count().exec(function(err,count){
                  ...
                  req.app.render('listpost',context,function(err,html){
                     if(err){
                         console.error('응답 웹문서 생성 중 오류발생:'+err.stack);
